...
file_raw_push = open('Data Collection/csv/raw/pushup.csv', 'a', newline='')
writer_raw_push = csv.writer(file_raw_push)

# EVENT (카운트 기반)
file_event_all = open('Data Collection/csv/event/reps_data.csv', 'a', newline='')
writer_event_all = csv.writer(file_event_all)

file_event_push = open('Data Collection/csv/event/pushup_reps.csv', 'a', newline='')
writer_event_push = csv.writer(file_event_push)
...
for f, w in [
    ...
    (file_raw_push, writer_raw_push),
    (file_event_all, writer_event_all),
    (file_event_push, writer_event_push)
]:
    if f.tell() == 0:
        w.writerow(headers)
...
cv2.namedWindow('Push-up Data Collector', cv2.WINDOW_NORMAL)
    ...
    while cap.isOpened():
        ...
        if angle is not None:
            ...
            if current_time - last_save_time > 0.1:
                ...
                writer_raw_push.writerow(row)
                last_save_time = current_time

            # EVENT 저장 (카운트 증가 시)
            if counter != prev_counter:
                writer_event_all.writerow(row)
                writer_event_push.writerow(row)
                prev_counter = counter
        
        # UI
        cv2.rectangle(image, (0,0), (250,80), (245,117,16), -1)
        
        cv2.putText(image, 'PUSH-UP', (10,20),
        ...
        cv2.imshow('Push-up Data Collector', resized)
